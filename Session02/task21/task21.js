...
TagPool.prototype={
	push:function(item){
		...
		if(this.data.indexOf(item)<0){
			...
			newNode.addEventListener('mouseover',this.onMouseOver)
			newNode.addEventListener('mouseout',this.onMouseOut)
			if(this.data.length>this.lengthLimit){
				this.data.shift()
				this.container.removeChild(this.container.children[0])
			}
			this.container.appendChild(newNode)
			newNode.addEventListener('click',this.deleteItem(1))
		}
	},
	onMouseOver:function(listItem){
		var node=listItem.target
		node.innerHTML='删除'+node.innerHTML
	},
	onMouseOut:function(listItem){
		var node=listItem.target
	...
	deleteItem:function(sender){
		var nodeToBeDeleted=item
		var parentNode=this.parentNode
		var deleteIndex=0
		for(var i=0;i<parentNode.children.length;i++){
			if(nodeToBeDeleted.isSameNode(parentNode.children[i])){
				deleteIndex=i;
				break;
			}
		}
